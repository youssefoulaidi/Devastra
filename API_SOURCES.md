...
> **شكل استجابة التفاسير (مهم):** القائمة `{"tafasir":[{"id":1,"name":"…"}]}`، أما مقاطع
> التفسير فهي `{"tafasir":{"name":"…","soar":[{"id":…,"tafsir_id":…,"name":"سورة البقرة الايات من 1 الى 25","url":"…","sura_id":2}]}}`
> — أي **`soar` مصفوفة** وليست كائن `sora`. الاعتماد على الشكل القديم كان يجعل قائمة
> التفاسير فارغة دائمًا. حاليًا يوفّر المصدر تفسيرًا صوتيًا واحدًا (الخلاصة من تفسير الطبري).
...
- **الأذان دون إنترنت**: 5 أصوات مضمّنة داخل المستودع في `app/src/main/assets/adhan/`
  (مكة، عبد الباسط، محمد رفعت، المنشاوي، صهيب خطبة) فتعمل دون إنترنت مباشرة.
  ويجلب البناء (مهمة `fetchAdhanAssets`) أصواتًا إضافية إن توفّرت الشبكة، وأي صوت
  آخر يمكن تحميله من داخل التطبيق إلى مساحة دائمة (`filesDir/adhan`) ليعمل دون إنترنت معًا.
- **مواقيت الصلاة دون إنترنت**: عند تعذّر الوصول إلى Aladhan تُحسب المواقيت فلكيًا على
  الجهاز (`data/PrayerCalc.java`) بنفس زوايا طريقة الحساب المختارة، وتُجدول تنبيهات الأذان
  لليوم والغد محليًا.
