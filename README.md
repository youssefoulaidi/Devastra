...
## 🆕 آخر الإصلاحات / Latest fixes

| المشكلة | الإصلاح |
|---|---|
| 🎧 **الملفات المحمّلة لا تعمل دون إنترنت** | كان مشغّل الصوت يستخدم `DefaultHttpDataSource` وهو لا يقرأ `file://`، فيفشل تشغيل الملفات المحمّلة. أصبح المشغّل يستخدم `DefaultDataSource` (يقرأ المحلي والشبكة) مع تحويل تلقائي بين الملف المحلي والبث عند أي خطأ |
...
| 🕌 **الأذان لا يعمل دون إنترنت** | 5 أصوات أذان مدمجة في التطبيق (مكة، العبدالباسط، رفعت، المنشاوي، صهيب) + إمكانية **تحميل أي صوت** للاستخدام دون إنترنت من إعدادات المؤذن، ويُحفظ في مساحة دائمة. وعند تعذّر الصوت يعمل نغمة التنبيه بدل الصمت |
